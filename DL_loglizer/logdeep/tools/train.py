...
from logdeep.dataset.sample import sliding_window, session_window, fixed_window
...
class Trainer():
    def __init__(self, model, options):
        ...
        if self.sample == 'sliding_window':
            ...
            val_logs, val_labels = sliding_window(self.data_dir,
                                              ...
                                              template_num=self.template_num,
                                              sample_ratio=0.001)
        #因为在数据预处理时，就已经将数据以滑动窗口的形式处理好了，所以这里只用固定窗口与数据预处理时的窗口大小相同即可（18）
        elif self.sample == 'fixed_window':
            train_logs, train_labels = fixed_window(self.data_dir,
                                                  datatype='train',
                                                  window_size=self.window_size,template_num=self.template_num)
            ...
            train_logs, train_labels = session_window(self.data_dir,
                                                      datatype='train',template_num=self.template_num)
            val_logs, val_labels = session_window(self.data_dir,
                                                  datatype='val',template_num=self.template_num)
        else:
        ...
        # self.model = nn.DataParallel(model,self.device)
    ...
    def train(self, epoch):
        ...
        for i, (log, label) in enumerate(tbar):
            ...
            # output = self.model(x_input=features)
            # output = self.model()
